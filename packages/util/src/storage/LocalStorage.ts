/**
 * LocalStorage 클래스는 브라우저의 로컬 스토리지를 쉽게 관리하기 위한 유틸리티 클래스.
 * 이 클래스를 사용하여 로컬 스토리지에 데이터를 저장하고, 조회하며, 삭제할 수 있음.
...
export default class LocalStorage {
  ...
  public getObject<T>(key?: string): T | null {
    const items: string | null = window.localStorage.getItem(key ?? this.__key);
  ...
  public setObject<T = object>(item: T, key?: string) {
    ...
    window.localStorage.setItem(key ?? this.__key, object);
  }

  /**
   * 지정된 키에 해당하는 로컬 스토리지 항목을 삭제.
   * @param {string} [key] - 삭제할 항목의 키. 제공되지 않을 경우 클래스에 설정된 키를 사용.
   */
  public clear(key?: string) {
    window.localStorage.removeItem(key ?? this.__key);
  }
}
